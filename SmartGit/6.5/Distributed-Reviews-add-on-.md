...
new database will be created and the [users database](#DistributedReviews(add-on)-review.users) will be populated
...
-   If the Pull Request is not yet ready for merge and/or there is
    ...
    [Commenting changes](#DistributedReviews(add-on)-review.commenting))
...
<a id="DistributedReviews(add-on)-review.commenting"></a>
...
garbage collecting the corresponding commits, see [Historic Commit Refs](#DistributedReviews(add-on)-review.historic-commit-refs). Only by
invoking **Delete** on a *closed* pull request, it will be removed from
the database and correspondings *refs* will be deleted.

<a id="DistributedReviews(add-on)-review.users"></a>
...
<a id="DistributedReviews(add-on)-review.historic-commit-refs"></a>
